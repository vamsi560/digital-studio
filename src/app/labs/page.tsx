...
function LabCard({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link href={href} className="block">
      <div className="relative bg-card border border-border rounded-xl p-8 flex flex-col items-center justify-center gap-6 w-60 h-48 hover:bg-secondary hover:border-primary transition-all duration-300 cursor-pointer shadow-lg hover:shadow-primary/10 transform hover:-translate-y-1">
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-8 text-primary bg-background">
      <div className="flex flex-col items-center text-center">
        <h1 className="text-5xl md:text-6xl font-bold my-16 motion-safe:animate-fade-in">
          Introducing
        </h1>
